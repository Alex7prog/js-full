...
export const createLogger = () => {
  ...
  const warn = msg => {
    logMemory.unshift({ message: msg, type: 'warn', dateTime: new Date() });
  };

  const error = msg => {
    logMemory.unshift({ message: msg, type: 'error', dateTime: new Date() });
  };

  const log = msg => {
    logMemory.unshift({ message: msg, type: 'log', dateTime: new Date() });
  };

  const getRecords = typeMsg => {
    if (!typeMsg) {
      return logMemory;
    }

    return logMemory.filter(obj => obj.type === typeMsg);
  };
